...
function AdminArtistasPanel({idGenero}) {
    ...
    async function loadElementos(){
        ...
        await fetch(`http://localhost:3000/api/artista/genero/${idGenero}`)
    ...
    return (
        <Container fluid className="p-5 bg-white min-vh-100">
            <Row className='d-flex justify-content-between align-items-center flex-row mb-5'>
               <h1 className='col-4'>Administracion Artistas</h1>
                <button className='btn btn-light border-dark col-2 p-3 fs-5' onClick={()=>navigate("/admin/artista/form/"+idGenero)}>Agregar Artista</button>
            </Row>
                    ...
                    {elementos.map((elemento) => (
                        <div className='list-group-item d-flex justify-content-between align-items-center flex-row p-3 bg-light' key={elemento.id}>
                            ...
                            <div className='col-3 d-flex gap-3'>
                                <button className='btn btn-light border-dark p-3 fs-5' onClick={()=>navigate(`/admin/album/${elemento.id}`)}>Ver categoria</button>
                                <button className='btn btn-primary border-dark p-3 fs-5' onClick={()=>navigate(`/admin/artista/form/${idGenero}/${elemento.id}`)}>Modificar</button>
...
export default AdminArtistasPanel