...
function AdminPanelGeneros() {

    const navigate = useNavigate();

    async function loadElementos(){
        await fetch(`http://localhost:3000/api/genero`)
    ...
    return (
        <Container fluid className="p-5 bg-white min-vh-100">
            <Row className='d-flex justify-content-between align-items-center flex-row mb-5'>
               <h1 className='col-4'>Administracion Generos</h1>
                <button className='btn btn-light border-dark col-2 p-3 fs-5' onClick={()=>navigate("/admin/form")}>Agregar Genero</button>
            </Row>
                    ...
                    {elementos.map((elemento) => (
                        <div className='list-group-item d-flex justify-content-between align-items-center flex-row p-3 bg-light' key={elemento.id}>
                            <div className='col-3 d-flex gap-5 d-flex align-items-center'>
                                ...
                                <img src={elemento.imagenUrl? `http://localhost:3000/${elemento.imagenUrl}` : defaultImg} className="img img-fluid col-3 border border-dark border-2 rounded-3" alt="imagen del genero" />
                            </div>
...
export default AdminPanelGeneros